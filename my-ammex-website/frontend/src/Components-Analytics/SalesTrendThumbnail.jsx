...
const SalesTrendThumbnail = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await getHistoricalSales(3); 
        if (!isMounted) return;
  ...
  // Generate mock chart data with clear uptrend
  useEffect(() => {
    const generateChartData = () => {
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        // Create strong uptrend with accelerating growth
        const baseValue = 580000 + ((3 - i) * 140000); // As i decreases (3→0), baseValue increases - proper uptrend
        const growthAcceleration = (3 - i) * 20000; // Additional acceleration factor
        const variation = Math.floor(Math.random() * 30000) - 15000; // ±15k smaller variation for cleaner trend
        
        months.push({
          month: monthName,
          sales: Math.max(550000, baseValue + growthAcceleration + variation) // Ensure minimum value
        });
      }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-blue-50/50 to-indigo-100/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">AI-powered sales predictions</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-gray-900">
              {isLoading ? 'Loading...' : thumbnailData.length > 0 ? formatCurrency(thumbnailData.reduce((sum, item) => sum + item.sales, 0)) : 'No data'}
            </p>
          </div>
          <div className="text-center">
            <div className="flex items-center justify-center gap-1 mb-1">
              <TrendingUp className="w-4 h-4 text-blue-600" />
              <span className="text-xs text-gray-600">Avg Monthly Sales</span>
            </div>
            <p className="text-lg font-bold text-blue-600">
              {isLoading ? 'Loading...' : thumbnailData.length > 0 ? formatCurrency(thumbnailData.reduce((sum, item) => sum + item.sales, 0) / thumbnailData.length) : 'No data'}
            </p>
          </div>
          <div className="text-center">
            <div className="flex items-center justify-center gap-1 mb-1">
              <BarChart3 className="w-4 h-4 text-purple-600" />
              <span className="text-xs text-gray-600">YTD Growth</span>
            </div>
          ...
          {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={120}>
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="thumbnailGradient" x1="0" y1="0" x2="0" y2="1">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs text-gray-600">
            ...
            <span>Historical sales analysis</span>
          </div>
          <div className="flex items-center gap-2 text-xs text-gray-600">
            <div className="w-2 h-2 bg-green-500 rounded-full"></div>
            <span>3-6 month forecasting</span>
          </div>
          <div className="flex items-center gap-2 text-xs text-gray-600">
        ...
        <div className="mt-4 pt-4 border-t border-gray-100">
          <div className="flex items-center justify-between">
            ...
            <div className="px-3 py-1 bg-blue-600 text-white text-xs rounded-full group-hover:bg-blue-700 transition-colors duration-300">
              Open Dashboard
            </div>
          </div>
        </div>
